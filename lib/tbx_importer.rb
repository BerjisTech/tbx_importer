  ...
  class Tbx
    ...
    def eval_state(tag_stack,reader)
      ...
      when [116, 101, 114, 109, 78, 111, 116, 101] #termNote
        @doc[:term][:part_of_speech] = PrettyStrings::Cleaner.new(reader.read_string.downcase).pretty.gsub("\\","&#92;").gsub("'",%q(\\\')).gsub("/",%q(\\\/)) if reader.get_attribute("type").eql?("partOfSpeech")
        @doc[:term][:vals].pop
        write_term(reader)
      when [100, 101, 115, 99, 114, 105, 112] #descrip
        @doc[:tc][:definition] = PrettyStrings::Cleaner.new(reader.read_string).pretty.gsub("\\","&#92;").gsub("'",%q(\\\')).gsub("/",%q(\\\/)) if reader.get_attribute("type").eql?("definition")
        @doc[:tc][:vals].pop
        write_tc
      end
    end
    ...
    def write_term(reader)
      ...
      text = PrettyStrings::Cleaner.new(reader.read_string).pretty.gsub("\\","&#92;").gsub("'",%q(\\\')).gsub("/",%q(\\\/))
